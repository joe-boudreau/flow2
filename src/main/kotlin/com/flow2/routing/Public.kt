...
fun Application.configurePublicRoutes() {
    ...
    routing {
        ...
        get("/projects") {
            val projects = listOf(
                mapOf(
                    "title" to "Daily Quordle Solver (2025)",
                    "link" to "/daily-quordle-solver.html",
                    "description" to "Automation running on AWS Lambda that solves Merriam-Webster's daily Quordle puzzle using Selenium and the OpenAI API. If it wins, art is generated by Dall-E using the four words from the puzzle."
                ),
                mapOf(
                    "title" to "SumarAI (2024)",
                    "link" to "https://chromewebstore.google.com/detail/sumarai/mkkjeikhldaeolgmcclafecidaggcacc",
                    "description" to "Chrome extension that enables one-click article summaries using the OpenAI API. Uses Mozilla's readability library for text extraction."
                ),
                mapOf(
                    "title" to "Chess Engine (2018)",
                    "link" to "https://github.com/joe-boudreau/chess-engine",
                    "description" to "A Java based chess engine implementing minimax search with alpha-beta tree pruning, transposition tables, iterative " +
                            "deepening, and heuristic board evaluations. Can be played using the built-in GUI or via the UCI protocol"
                ),
            )
